angular.module('hocketWebsite')
     .controller('Myrilux_1BHK_Combo_B_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Myrilux_1BHK_Combo_B_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/1BHK/6.png';
            ...
            $scope.tarrif_rate = [0,2764,2414,2298,2239,2204,2181,2164,2151,2142,2134,2128,2123,2118,2114,2111,2108,2106,2103];
            $scope.itemsIncluded = "4 Core + 6 Addon items";
            $scope.securityDeposit = 2500;
            $scope.productId = 'CAF1BMB';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Myrilux 1BHK Combo - B',
