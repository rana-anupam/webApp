angular.module('hocketWebsite')
     .controller('Cowboy_3BHK_Combo_B_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_3BHK_Combo_B_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/2.png';
            ...
            $scope.tarrif_rate = [0,3963,3513,3363,3288,3243,3213,3192,3176,3163,3153,3145,3138,3132,3127,3123,3119,3116,3113];
            $scope.itemsIncluded = "5 Core + 12 Addon items";
            $scope.securityDeposit = 3500;
            $scope.productId = 'CAF3BCB';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 3BHK Combo - B',
