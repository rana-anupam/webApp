angular.module('hocketWebsite')
     .controller('Double_Door_Fridge_plus_Front_Load_WM_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Double_Door_Fridge_plus_Front_Load_WM_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/appliances/top_load_wm/top_load_wm.jpg';
            ...
            $scope.tarrif_rate = [0,2080,1730,1614,1555,1520,1497,1480,1467,1458,1450,1444,1439,1434,1430,1427,1424,1422,1419];
            $scope.itemsIncluded = "2 Core items";
            $scope.securityDeposit = 1900;
                  $scope.productId = 'CA05';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Double Door Refrigerator + Front Load Washine Machine',
