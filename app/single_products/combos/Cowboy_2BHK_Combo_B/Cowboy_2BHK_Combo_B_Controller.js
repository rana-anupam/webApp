angular.module('hocketWebsite')
     .controller('Cowboy_2BHK_Combo_B_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_2BHK_Combo_B_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/2BHK/2.png';
            ...
            $scope.tarrif_rate = [0,2692,2292,2159,2092,2052,2025,2006,1992,1981,1972,1965,1959,1954,1949,1945,1942,1939,1937];
            $scope.itemsIncluded = "3 Core + 10 Addon items";
            $scope.securityDeposit = 2300;
            $scope.productId = 'CAF2BCB';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 2BHK Combo - B',
