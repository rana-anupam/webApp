angular.module('hocketWebsite')
     .controller('Myrilux_3BHK_Combo_A_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Myrilux_3BHK_Combo_A_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/5.png';
            ...
            $scope.tarrif_rate = [0,4455,4005,3855,3780,3735,3705,3684,3668,3655,3645,3637,3630,3624,3619,3615,3611,3608,3605];
            $scope.itemsIncluded = "5 Core + 12 Addon items";
            $scope.securityDeposit = 4000;
            $scope.productId = 'CAF3BMA';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Myrilux 3BHK Combo - A',
