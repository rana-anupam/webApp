angular.module('hocketWebsite')
     .controller('Myrilux_2BHK_Combo_A_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Myrilux_2BHK_Combo_A_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/2BHK/5.png';
            ...
            $scope.tarrif_rate = [0,3520,3120,2987,2920,2880,2853,2834,2820,2809,2800,2793,2787,2782,2777,2773,2770,2767,2765];
            $scope.itemsIncluded = "4 Core + 8 Addon items";
            $scope.securityDeposit = 3100;
            $scope.productId = 'CAF2BMA';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Myrilux 2BHK Combo - A',
