angular.module('hocketWebsite')
     .controller('Cowboy_1BHK_Combo_C_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_1BHK_Combo_C_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/1BHK/3.png';
            ...
            $scope.tarrif_rate = [0,2421,2071,1955,1896,1861,1838,1821,1808,1799,1791,1785,1780,1775,1771,1768,1765,1763,1760];
            $scope.itemsIncluded = "4 Core + 6 Addon items";
            $scope.securityDeposit = 2000;
            $scope.productId = 'CAF1BCC';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 1BHK Combo - C',
