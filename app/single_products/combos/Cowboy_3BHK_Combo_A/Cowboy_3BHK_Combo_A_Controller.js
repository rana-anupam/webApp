angular.module('hocketWebsite')
     .controller('Cowboy_3BHK_Combo_A_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_3BHK_Combo_A_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/5.png';
            ...
            $scope.tarrif_rate = [0,3463,3013,2863,2788,2743,2713,2692,2676,2663,2653,2645,2638,2632,2627,2623,2619,2616,2613];
            $scope.itemsIncluded = "5 Core + 12 Addon items";
            $scope.securityDeposit = 3000;
            $scope.productId = 'CAF3BCA';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 3BHK Combo - A',
