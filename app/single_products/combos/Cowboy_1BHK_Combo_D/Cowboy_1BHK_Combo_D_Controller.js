angular.module('hocketWebsite')
     .controller('Cowboy_1BHK_Combo_D_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_1BHK_Combo_D_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/1BHK/4.png';
            ...
            $scope.tarrif_rate = [0,2534,2184,2068,2009,1974,1951,1934,1921,1912,1904,1898,1893,1888,1884,1881,1878,1876,1873];
            $scope.itemsIncluded = "4 Core + 6 Addon items";
            $scope.securityDeposit = 2200;
            $scope.productId = 'CAF1BCD';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 1BHK Combo - D',
