angular.module('hocketWebsite')
     .controller('Cowboy_3BHK_Combo_D_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_3BHK_Combo_D_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/4.png';
            ...
            $scope.tarrif_rate = [0,4302,3852,3702,3627,3582,3552,3531,3515,3502,3492,3484,3477,3471,3466,3462,3458,3455,3452];
            $scope.itemsIncluded = "8 Core + 18 Addon items";
            $scope.securityDeposit = 3900;
            $scope.productId = 'CAF3BCD';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 3BHK Combo - D',
