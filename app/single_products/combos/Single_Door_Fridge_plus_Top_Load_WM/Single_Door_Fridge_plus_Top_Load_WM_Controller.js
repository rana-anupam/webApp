angular.module('hocketWebsite')
     .controller('Single_Door_Fridge_plus_Top_Load_WM_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Single_Door_Fridge_plus_Top_Load_WM_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/Appliances/2.png';
       	...
       	$('.bannerImage').attr("src",$scope.bannerImageSrc);
            ...
            $scope.securityDeposit = 1400;
            $scope.productId = 'CA02';

            $scope.tarrif_rate = [0,1750,1400,1284,1225,1190,1167,1150,1137,1128,1120,1114,1109,1104,1100,1097,1094,1092,1089];
       	if(package == 'basic'){
       	...
       	var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
       		if($scope.user.loggedIn){
       			...
       			var request = {			userId : $scope.user.id,
		   								...
		   								productName: 'Single Door Refrigerator + Top Load Washine Machine',
