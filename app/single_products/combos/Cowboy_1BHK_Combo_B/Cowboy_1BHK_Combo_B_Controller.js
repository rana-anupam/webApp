angular.module('hocketWebsite')
     .controller('Cowboy_1BHK_Combo_B_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_1BHK_Combo_B_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/1BHK/2.png';
            ...
            $scope.tarrif_rate = [0,1921,1571,1455,1396,1361,1338,1321,1308,1299,1291,1285,1280,1275,1271,1268,1265,1263,1260];
            $scope.itemsIncluded = "2 Core + 4 Addon items";
            $scope.securityDeposit = 1500;
            $scope.productId = 'CAF1BCB';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 1BHK Combo - B',
