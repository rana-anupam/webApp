angular.module('hocketWebsite')
     .controller('Cowboy_3BHK_Combo_C_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_3BHK_Combo_C_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/3.png';
            ...
            $scope.tarrif_rate = [0,4293,3843,3693,3618,3573,3543,3522,3506,3493,3483,3475,3468,3462,3457,3453,3449,3446,3443];
            $scope.itemsIncluded = "5 Core + 12 Addon items";
            $scope.securityDeposit = 3900;
            $scope.productId = 'CAF3BCC';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 3BHK Combo - C',
