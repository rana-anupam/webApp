angular.module('hocketWebsite')
     .controller('Myrilux_3BHK_Combo_B_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Myrilux_3BHK_Combo_B_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/3BHK/6.png';
            ...
            $scope.tarrif_rate = [0,4992,4542,4392,4317,4272,4242,4221,4205,4192,4182,4174,4167,4161,4156,4152,4148,4145,4142];
            $scope.itemsIncluded = "8 Core + 18 Addon items";
            $scope.securityDeposit = 4600;
            $scope.productId = 'CAF3BMB';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Myrilux 3BHK Combo - B',
