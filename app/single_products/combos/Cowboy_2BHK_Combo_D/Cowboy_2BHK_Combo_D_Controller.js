angular.module('hocketWebsite')
     .controller('Cowboy_2BHK_Combo_D_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_2BHK_Combo_D_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/2BHK/4.png';
            ...
            $scope.tarrif_rate = [0,3418,3018,2885,2818,2778,2751,2732,2718,2707,2698,2691,2685,2680,2675,2671,2668,2665,2663];
            $scope.itemsIncluded = "6 Core + 12 Addon items";
            $scope.securityDeposit = 3000;
            $scope.productId = 'CAF2BCD';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 2BHK Combo - D',
