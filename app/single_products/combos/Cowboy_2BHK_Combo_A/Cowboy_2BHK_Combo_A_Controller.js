angular.module('hocketWebsite')
     .controller('Cowboy_2BHK_Combo_A_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_2BHK_Combo_A_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/2BHK/1.png';
            ...
            $scope.tarrif_rate = [0,2542,2142,2008,1942,1902,1875,1856,1842,1830,1822,1814,1808,1803,1799,1795,1792,1789,1787];
            $scope.itemsIncluded = "3 Core + 10 Addon items";
            $scope.securityDeposit = 2100;
            $scope.productId = 'CAF2BCA';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 2BHK Combo - A',
