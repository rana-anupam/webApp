angular.module('hocketWebsite')
     .controller('Double_Door_Fridge_plus_Top_Load_WM_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Double_Door_Fridge_plus_Top_Load_WM_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/Appliances/3.png';
            ...
            $scope.tarrif_rate = [0,2000,1650,1534,1475,1440,1417,1400,1387,1378,1370,1364,1359,1354,1350,1347,1344,1342,1339];
            $scope.itemsIncluded = "2 Core items";
            $scope.securityDeposit = 1400;
                  $scope.productId = 'CA03';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Double Door Refrigerator + Top Load Washine Machine',
