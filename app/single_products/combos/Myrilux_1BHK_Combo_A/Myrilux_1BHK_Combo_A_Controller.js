angular.module('hocketWebsite')
     .controller('Myrilux_1BHK_Combo_A_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Myrilux_1BHK_Combo_A_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/1BHK/5.png';
            ...
            $scope.tarrif_rate = [0,2585,2235,2119,2060,2025,2002,1985,1972,1963,1955,1949,1944,1939,1935,1932,1929,1927,1924];
            $scope.itemsIncluded = "3 Core + 4 Addon items";
            $scope.securityDeposit = 2300;
            $scope.productId = 'CAF1BMA';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Myrilux 1BHK Combo - A',
