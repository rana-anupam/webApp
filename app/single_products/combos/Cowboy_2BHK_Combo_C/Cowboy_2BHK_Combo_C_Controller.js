angular.module('hocketWebsite')
     .controller('Cowboy_2BHK_Combo_C_Controller', ['$scope','$state','$http', function($scope, $state,$http){
       console.log("Cowboy_2BHK_Combo_C_Controller controller is loading");
       ...
       $scope.activePackage = function(package){
        ...
        $scope.bannerImageSrc = './app/single_products/combos/images/2BHK/3.png';
            ...
            $scope.tarrif_rate = [0,3192,2792,2659,2592,2552,2525,2506,2492,2481,2472,2465,2459,2454,2449,2445,2442,2439,2437];
            $scope.itemsIncluded = "4 Core + 10 Addon items";
            $scope.securityDeposit = 2800;
            $scope.productId = 'CAF2BCC';
            if(package == 'basic'){
            ...
            var time = moment().add({hours:5,minutes:30}).format('MMMM Do YYYY , h:mm:ss a');
                  if($scope.user.loggedIn){
                        ...
                        var request = {               userId : $scope.user.id,
                                                            ...
                                                            productName: 'Cowboy 2BHK Combo - C',
